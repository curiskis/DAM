...
c(1, 2, 3) * 1
```
R interprets this command as adding the vector of 1's to the vector `(1, 2, 3)`.  For those who remember their linear algebra, we can also run vector operations easily in R by using the '`%`' notation. For example, the dot product of vectors `(1, 2, 3)`:
```{R c product}
c(1, 2, 3) %*% c(1, 2, 3)
```
...
To bring up the help documentation for a function in R, you can either run __`help()`__, or put a __`?`__ in front of the function name. Both are equivalent:
```{R help}
help(seq)
?seq
```

There is a commonly used shorthand for __`seq()`__ where the _`by`_ increment is always 1:
```{R :}
1:20
```
We can also create a vector of a value replicate any number of times using __`rep()`__:
```{R rep}
rep(1, 20)
```
...
```
Note that the arguments to functions like __`seq()`__ and __`rep()`__ do not need to be named explicitly (e.g. from = ...).  However, if you do not name them then you must input them in the same order as the documentation.  Note also that R has given each column (or variable) a name in line with the vector definition. We usually rename the variables in our data frames to something nicer.  There are a few equivalent ways to do this: __`setNames()`__, with our data frame as the input, we can assign a name to each vector input to the data frame, or assign a new vector to __`colnames()`__, again with our data frame as the input (this third option requires that we assign the data frame to an object, explained in the next section):
```{R df2, eval=FALSE}
...
The other common way to subset data frames is to explicitly use the row or column names.  Remember that in R, every value is defined as a vector.  If we want to subset by multiple rows or columns, we must give their names as a vector.  For instance, 
